...
- Las encuestas realizadas para el análisis político y electoral suelen partir de muestras cuasiprobabilísticas o no probabilísticas.

- En España las encuesta telefónicas utilizan cuotas de sexo y edad. Algunas encuestas de panel online utilizan cuotas más extendidas.

- En algunas encuestas telefónicas se especifica el uso de teléfonos fijos y móviles.
...
Según su función y su manera de generarlos hay diferentes tipos de pesos:
...
- Puede ocurrir que algunas categorías pueden tener un **número reducido de casos** (n < 10), por lo que cabe la posibilidad de combinar las celdas para evitar pesos extremos.
...
- Declarar el **diseño** de la encuesta con `svydesign()`. Esta función sirve para registrar cuál es el diseño de la encuesta:
  ...
  - `ids` es la variable conglomerados en el caso de que la meustra sea conglomerada. En caso contrario utilizar `~ 0`.
...
- Generar los **pesos de calibración** con la función `calibrate()`:
  ...
  - `formula` son las varaibles incluidas en la calibración antecedidas por `~`.
...
- La **falta de representatividad no invalida el uso de los datos**, pero hay que tenerlas en cuenta a la hora de lelvar a cabo el análisis.
...
- El funcionamiento de los ajuste es parcial y dependen en lo **adecuadas que sean las variables auxiliares**. Las variables auxiliares deben explicar el mecanismo de respuesta y estár correlacionadas con las variables de interés.
